...
public class Main {
    ...
    public static void isPerfect(long n, StringBuilder builder) {
        long sum = 1;
        for (int i = 2; i * i <= n; i++) {
            if (n % i == 0) {
                sum += i; 
        ...
        if (sum == n) {
            builder.append(n + " perfect\n");
        } else if (Math.abs(sum - n) <= 2) {
            builder.append(n + " almost perfect\n");
        } else {
            builder.append(n + " not perfect\n");
        }
    }
