

import java.util.*;
import java.io.*;

public class Main {

    static boolean readFile = false;
    static boolean[] primes;
    static boolean[] already;
    static int count = 0;
    public static boolean[] generatePrimes(int n) {
        boolean[] prime = new boolean[n + 1];
        Arrays.fill(prime, 2, n + 1, true);
        for (int i = 2; i * i <= n; i++) {
            if (prime[i]) {
                for (int j = i * i; j <= n; j += i) {
                    prime[j] = false;
                }
            }
        }
        return prime;
    }
    public static void partition(int[] binary, int pos, int[] arr) {
        if (pos <= binary.length - 1) {
            partition(binary, pos + 1, arr);
            binary[pos] = 1;
            ArrayList<Integer> subset = new ArrayList<>();
            for (int i = 0; i < binary.length; ++i) {
                if (binary[i] == 1) {
                    subset.add(arr[i]);
                }
            }
            Integer[] temp = new Integer[subset.size()];
            temp = subset.toArray(temp);
            permute(temp, 0);
            partition(binary, pos + 1, arr);
            binary[pos] = 0;
        }
    }

    private static void permute(Integer[] arr, int index) {
        int n = arr.length;
        if (index >= n - 1) { //If we are at the last element - nothing left to permute
            int number = 0;
            for (int i : arr) {
                number = number*10 + i;
            }
            if(primes[number] && !already[number]){
               already[number] = true;
               count++;
            }
            return;
        }
        for (int i = index; i < n; ++i) { //For each index in the sub array arr[index...end]
            //Swap the elements at indices index and i
            int temp = arr[index];
            arr[index] = arr[i];
            arr[i] = temp;
            //Recurse on the sub array arr[index+1...end]
            permute(arr, index + 1);
            //Swap the elements back
            temp = arr[index];
            arr[index] = arr[i];
            arr[i] = temp;
        }
    }

    public static void main(String[] args) throws IOException {
        InputReader ip = new InputReader();
        primes = generatePrimes(10000000);
        StringBuilder builder = new StringBuilder();
        int n = ip.nextInt();
        for(int t =0; t <n; ++t){
            String s = ip.next();
            count = 0;
            already = new boolean[10000000];
            int[] arr = new int[s.length()];
            for (int i = 0; i < s.length(); ++i) {
                arr[i] = Integer.parseInt(String.valueOf(s.charAt(i)));
            }
            partition(new int[s.length()], 0, arr);
            builder.append(count +"\n");
        }
        System.out.print(builder);
        
    }

    static class InputReader {

        InputStream is = System.in;
        byte[] inbuf = new byte[1 << 23];
        int lenbuf = 0, ptrbuf = 0;

        public InputReader() throws IOException {
            lenbuf = is.read(inbuf);
        }

        public int readByte() {

            if (ptrbuf >= lenbuf) {
                return -1;
            }

            return inbuf[ptrbuf++];
        }

        public boolean hasNext() {
            int t = skip();

            if (t == -1) {
                return false;
            }
            ptrbuf--;
            return true;
        }

        public boolean isSpaceChar(int c) {
            return !(c >= 33 && c <= 126);
        }

        public int skip() {
            int b;
            while ((b = readByte()) != -1 && isSpaceChar(b))
                ;
            return b;
        }

        public double nextDouble() {
            return Double.parseDouble(next());
        }

        public char nextChar() {
            return (char) skip();
        }

        public String next() {
            int b = skip();
            StringBuilder sb = new StringBuilder();
            while (!(isSpaceChar(b))) {
                sb.appendCodePoint(b);
                b = readByte();
            }
            return sb.toString();
        }

        public char[] ns(int n) {
            char[] buf = new char[n];
            int b = skip(), p = 0;
            while (p < n && !(isSpaceChar(b))) {
                buf[p++] = (char) b;
                b = readByte();
            }
            return n == p ? buf : Arrays.copyOf(buf, p);
        }

        public char[][] nm(int n, int m) {
            char[][] map = new char[n][];
            for (int i = 0; i < n; i++) {
                map[i] = ns(m);
            }
            return map;
        }

        public int[] na(int n) {
            int[] a = new int[n];
            for (int i = 0; i < n; i++) {
                a[i] = nextInt();
            }
            return a;
        }

        public int nextInt() {
            int num = 0, b;
            boolean minus = false;
            while ((b = readByte()) != -1 && !((b >= '0' && b <= '9') || b == '-'))
                ;
            if (b == '-') {
                minus = true;
                b = readByte();
            }

            while (true) {
                if (b >= '0' && b <= '9') {
                    num = num * 10 + (b - '0');
                } else {
                    return minus ? -num : num;
                }
                b = readByte();
            }
        }

        public long nextLong() {
            long num = 0;
            int b;
            boolean minus = false;
            while ((b = readByte()) != -1 && !((b >= '0' && b <= '9') || b == '-'))
                ;
            if (b == '-') {
                minus = true;
                b = readByte();
            }

            while (true) {
                if (b >= '0' && b <= '9') {
                    num = num * 10 + (b - '0');
                } else {
                    return minus ? -num : num;
                }
                b = readByte();
            }
        }

    }
}
