...
public class SO_CACH_CHATCAY {
	...
	public static void main(String[] args) throws IOException {
		...
		builder.append(find(arr, n) + " " + hm.get(n - 1) % 1000000007);
	...
	static long find(long tree[], int n) {
		...
		hm.put(2, 1L);
		for (int i = 2; i < n; i++) {
			dp[i] = Math.max(tree[i] + dp[i - 2], dp[i - 1]);
			if (i >= 3) {
				if (dp[i] == dp[i - 1]) {
					hm.put(i, hm.get(i - 2) + hm.get(i - 1));
				} else {
					hm.put(i, hm.get(i - 2));
				}
				if (tree[i] + tree[i - 2] == dp[i]) {
					hm.put(i, 1 + hm.get(i - 2) + hm.get(i - 1));
				}
			}
		}
		return dp[n - 1];
